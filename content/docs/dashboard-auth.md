...
Put the Rollout Dashboard behind your OIDC provider. A single [oauth2-proxy](https://oauth2-proxy.github.io/oauth2-proxy/) deployment in `auth-system` gates the dashboard (and any other service you point at it) via [Envoy Gateway](https://gateway.envoyproxy.io) extAuth.
...
Both the dashboard and the Kubernetes API server use this same client id, so the id_token authenticates against both.
...
To gate another service, apply the same `SecurityPolicy` shape in its namespace and extend the `ReferenceGrant` `from` list to include that namespace.
